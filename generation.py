...
def generate(seed=None, agent_name=None):
    ...
    goalSample = [goal for goal in parse_goals(
        domain_str) if goal.split()[0] not in ["(not", "(character"]]
    if len(goalSample) == 0:
    ...
    for i in range(goalN):
        ...
        for j in goals:
            j2 = j.split()
            g2 = g.split()
            # For comparison, not doesnt matter if equal
            if g2[0] == "(not" and j2[0] == "(not":
                g2 = g2[1:]
                j2 = j2[1:]
